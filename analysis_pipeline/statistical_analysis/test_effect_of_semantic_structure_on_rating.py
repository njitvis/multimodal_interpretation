...
df = pd.merge(rating, semantic_structure, on="image_id", how="left")
...
def bin_rating(x): 
    if x < 3:
        ...
        return 1
df["rating_bin"] = df["mean_rating"].apply(bin_rating)
...
res_clr_interval = res_clr.conf_int(0.05)

print(res_clr.summary())
print(res_clr_interval)
...
for i,coef in enumerate(res_clr.params[1:], start=1):
    if coef == 0:
        ...
        print(f"increasing proportion of L{i} sentence {"increases" if coef > 0 else "decreases"} rating by {np.exp(coef):.3f}")
