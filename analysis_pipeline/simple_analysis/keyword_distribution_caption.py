...
labeled_sentences = pd.read_csv('./all.csv')
labeled_sentences = labeled_sentences[["text", "label"]].copy()
labeled_sentences.rename(columns={"text": "sentence", "label": "semantic_level"}, inplace = True)
labeled_sentences["semantic_level"] = labeled_sentences["semantic_level"].apply(ast.literal_eval)
labeled_sentences = labeled_sentences.explode('semantic_level', ignore_index=True)
...
plt.tight_layout()
plt.savefig("./output/keyword_distribution_captions.png", dpi=300)
plt.close()
